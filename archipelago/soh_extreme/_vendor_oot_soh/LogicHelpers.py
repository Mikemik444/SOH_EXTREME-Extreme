...
def extreme_requirement(bundle, option_name: str, item_name: str) -> Rule:
    """An unshuffled capability is innate; a shuffled capability needs its item.

    Used at the action itself, including event/resource rules. A location-only
    overlay cannot prevent a refill or room event from assuming a free action.
    """
    option = getattr(bundle[1].options, option_name, None)
    return Has(item_name) if option is not None and option.value else True_()


def can_grab(bundle) -> Rule:
    return extreme_requirement(bundle, "shuffle_grab", "Grab / Power Bracelet")


def can_swim(bundle) -> Rule:
    return extreme_requirement(bundle, "shuffle_swim", "Swim")


def can_break_rocks(bundle) -> Rule:
    """Native CanBreakRocks: existence soul AND explosives or basic Grab.

    Do not change blast_or_smash: mud walls and other non-rock obstacles use it.
    """
    return extreme_requirement(bundle, "shuffle_rock_boulder_soul", "Rock / Boulder Soul") & (
        has_explosives(bundle) | can_grab(bundle))


def can_climb(bundle) -> Rule:
    return extreme_requirement(bundle, "shuffle_climb", "Climb")


def can_crawl(bundle) -> Rule:
    return extreme_requirement(bundle, "shuffle_crawl", "Crawl")



def has_animal_soul(bundle, animal: str) -> Rule:
    """Match native HasAnimalSoul: disabled, shared, or the exact species."""
    mode = getattr(bundle[1].options, "shuffle_animal_soul", None)
    if mode is None or not mode.value:
        return True_()
...
def can_use(item: Items, bundle: tuple[Regions, "SohWorld"]) -> Rule:
    ...
    if item == Items.IRON_BOOTS:
        # EXTREME voids Link on water contact without Swim, including in Irons.
    ...
    if item == Items.FISHING_POLE:
        ...
        # Bottled overworld/grotto fish never use the rod and are unaffected.
        rule &= has_item(Items.CHILD_WALLET, bundle) & can_interact_npc(bundle, "Hylian")
        animal_mode = getattr(bundle[1].options, "shuffle_animal_soul", None)
        if animal_mode is not None and animal_mode.value:
            rule &= Has("Animal Soul" if animal_mode.value == 1 else "Fish Soul")

    if item == Items.EPONA:
        # Riding is an action, not possession of the song or a historical race
        # event. This helper is shared by every bridge/fence/minigame route.
        rule &= is_adult(bundle) & can_play_song(Items.EPONAS_SONG, bundle)
        mode = getattr(bundle[1].options, "shuffle_animal_soul", None)
        if mode is not None and mode.value:
            rule &= Has("Animal Soul" if mode.value == 1 else "Horse Soul")
...
def has_item(item: Items | Events | StrEnum, bundle: tuple[Regions, "SohWorld"], count: int = 1) -> Rule:
    ...
    for prog,non_progs in progressive_items.items():
        if item in non_progs:
            rule = Has(prog, non_progs.index(item) + 1)
            if item in (Items.GORONS_BRACELET, Items.SILVER_GAUNTLETS, Items.GOLDEN_GAUNTLETS):
                rule &= can_grab(bundle)
            return rule
...
def scarecrows_song(bundle: tuple[Regions, "SohWorld"]) -> Rule:
    # Teaching the tune does not provide an instrument/buttons to play it.
    return (can_use(Items.FAIRY_OCARINA, bundle) & has_enough_ocarina_buttons(bundle, 2)
            & (OptionFilter(SkipScarecrowsSong, True)
               | (has_item(Events.CHILD_SCARECROW_UNLOCKED, bundle)
                  & has_item(Events.ADULT_SCARECROW_UNLOCKED, bundle))))
...
def can_play_song(song: StrEnum, bundle: tuple[Regions, "SohWorld"]) -> Rule:
    # Full-song possession is produced by collect/remove only after the complete
    # note group when Individual Notes is enabled. The instrument and physical
    # buttons remain independent prerequisites even after that song is unlocked.
    world = bundle[1]
    note_mode = getattr(world.options, "song_note_shuffle", None)
    learned = has_item(song, bundle)
    if note_mode is not None and note_mode.value:
        # Do not let a stale or manually supplied full-song event bypass a
        # missing note. Completion, instrument, and buttons are independent.
        learned &= HasAll(*world.SONG_NOTE_GROUPS[str(song)])
    return (can_use(Items.FAIRY_OCARINA, bundle) & learned
            & (OptionFilter(ShuffleOcarinaButtons, False)
               | HasAll(*ocarina_buttons_required[song])))
...
def _time_access(bundle, want_day: bool) -> Rule:
    cycle = ((is_child(bundle) & has_item(Events.CHILD_CAN_PASS_TIME, bundle))
             | (is_adult(bundle) & has_item(Events.ADULT_CAN_PASS_TIME, bundle)))
    options = bundle[1].options
    flow = getattr(options, "shuffle_flow_of_time", None)
...
def at_day(bundle: tuple[Regions, "SohWorld"]) -> Rule:
    return _time_access(bundle, True)


def at_night(bundle: tuple[Regions, "SohWorld"]) -> Rule:
    return _time_access(bundle, False)
...
def can_break_crates(bundle: tuple[Regions, "SohWorld"]) -> Rule:
    """A fixed crate needs its soul and Roll, explosives, or usable Hammer."""
    return extreme_requirement(bundle, "shuffle_crate_soul", "Crate Soul") & (
        extreme_requirement(bundle, "shuffle_roll", "Roll") |
        has_explosives(bundle) | can_use(Items.MEGATON_HAMMER, bundle))


def can_break_small_crates(bundle: tuple[Regions, "SohWorld"]) -> Rule:
    """Small crates also support lifting and jumpslashes."""
    return extreme_requirement(bundle, "shuffle_crate_soul", "Crate Soul") & (
        extreme_requirement(bundle, "shuffle_roll", "Roll") | can_grab(bundle) |
        has_explosives(bundle) | can_jump_slash(bundle))


def can_bonk_trees(bundle: tuple[Regions, "SohWorld"]) -> Rule:
    """A tree soul does not supply the missing Roll action."""
    return extreme_requirement(bundle, "shuffle_tree_soul", "Tree Soul") & \
        extreme_requirement(bundle, "shuffle_roll", "Roll")
...
def can_break_lower_hives(bundle: tuple[Regions, "SohWorld"]) -> Rule:
    return extreme_requirement(bundle, "shuffle_beehive_soul", "Beehive Soul") & (
        can_break_upper_beehives(bundle) | can_use(Items.BOMB_BAG, bundle))


def can_break_upper_beehives(bundle: tuple[Regions, "SohWorld"]) -> Rule:
    return extreme_requirement(bundle, "shuffle_beehive_soul", "Beehive Soul") & (hookshot_or_boomerang(bundle) |
...
def _can_kill_enemy_combat(bundle: tuple[Regions, "SohWorld"], enemy: Enemies, distance: EnemyDistance = EnemyDistance.CLOSE,
...
def has_enough_ocarina_buttons(bundle: tuple[Regions, "SohWorld"], amount: int) -> Rule:
    ...
    # Duplicate copies of one button never make two playable pitches.
    return OptionFilter(ShuffleOcarinaButtons, False) | HasGroupUnique(group_name, amount)
...
def water_timer_at_least(bundle: tuple[Regions, "SohWorld"], amount: int) -> Rule:
    return can_swim(bundle) & (can_use(Items.ZORA_TUNIC, bundle) | (
        HeartsAtLeast(amount=int(math.ceil(amount/8))) & can_do_trick(Tricks.FEWER_TUNIC_REQUIREMENTS, bundle)))
...
def can_trigger_lacs(bundle: tuple[Regions, "SohWorld"]) -> Rule:
    if bundle[1].options.triforce_hunt.value:
...
# SOH-EXTREME 0.11.12: use the same per-species souls for traversal and enemy
# locations. A room-clear event may not certify kills while its enemies are absent.
_EXTREME_ENEMY_SOULS = {
    Enemies.STALFOS: 'Stalfos Soul',
    Enemies.OCTOROK: 'Octorok Soul',
    Enemies.WALLMASTER: 'Wallmaster Soul',
    Enemies.DODONGO: 'Dodongo Soul',
    Enemies.KEESE: 'Keese Soul',
    Enemies.FIRE_KEESE: 'Keese Soul',
    Enemies.LIZALFOS: 'Lizalfos and Dinolfos Soul',
    Enemies.DINOLFOS: 'Lizalfos and Dinolfos Soul',
    Enemies.GOHMA_LARVA: 'Gohma Larva Soul',
    Enemies.SHABOM: 'Shabom Soul',
    Enemies.BARI: 'Biri and Bari Soul',
    Enemies.TORCH_SLUG: 'Torch Slug Soul',
    Enemies.ARMOS: 'Armos Soul',
    Enemies.DEKU_BABA: 'Deku Baba Soul',
    Enemies.WITHERED_DEKU_BABA: 'Deku Baba Soul',
    Enemies.MAD_SCRUB: 'Deku Scrub Soul',
    Enemies.BLUE_BUBBLE: 'Bubble Soul',
    Enemies.GREEN_BUBBLE: 'Bubble Soul',
    Enemies.BEAMOS: 'Beamos Soul',
    Enemies.FLOORMASTER: 'Floormaster Soul',
    Enemies.REDEAD: 'Redead and Gibdo Soul',
    Enemies.GIBDO: 'Redead and Gibdo Soul',
    Enemies.FLARE_DANCER: 'Flare Dancer Soul',
    Enemies.DEAD_HAND: 'Dead Hand Soul',
    Enemies.SHELL_BLADE: 'Shell Blade Soul',
    Enemies.LIKE_LIKE: 'Like Like Soul',
    Enemies.SPIKE: 'Spike Soul',
    Enemies.ANUBIS: 'Anubis Soul',
    Enemies.IRON_KNUCKLE: 'Iron Knuckle Soul',
    Enemies.FREEZARD: 'Freezard Soul',
    Enemies.STINGER: 'Stinger Soul',
    Enemies.WOLFOS: 'Wolfos Soul',
    Enemies.WHITE_WOLFOS: 'Wolfos Soul',
    Enemies.TENTACLE: 'Jabu Jabu Tentacle Soul',
    Enemies.DARK_LINK: 'Dark Link Soul',
    Enemies.GERUDO_WARRIOR: 'Gerudo Thief Soul',
    Enemies.MEG: 'Poe Sister Soul',
    Enemies.BIG_OCTO: 'Big Octo Soul',
    Enemies.PURPLE_LEEVER: 'Leever Soul',
}

def has_extreme_enemy_soul(bundle, enemy):
    options = bundle[1].options
    if enemy in (Enemies.GOLD_SKULLTULA, Enemies.BIG_SKULLTULA):
        option = getattr(options, "shuffle_skulltula_soul", None)
        return Has("Skulltula Soul") if option is not None and option.value else True_()
    mode = getattr(options, "shuffle_enemy_soul", None)
    if mode is None or not mode.value:
        return True_()
    if mode.value == 1:
        return Has("Enemy Soul")
    soul = _EXTREME_ENEMY_SOULS.get(enemy)
    return Has(soul) if soul is not None else True_()

def can_kill_enemy(bundle, enemy, distance=EnemyDistance.CLOSE, wall_or_floor=True,
                   quantity=1, timer=False, in_water=False):
    return has_extreme_enemy_soul(bundle, enemy) & _can_kill_enemy_combat(
        bundle, enemy, distance, wall_or_floor, quantity, timer, in_water)
