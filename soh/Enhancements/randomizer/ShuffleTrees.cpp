...
static bool GrassSanityIncludesBushes() {
    const auto grassSetting = Rando::Context::GetInstance()->GetOption(RSK_SHUFFLE_GRASS);
    return grassSetting.Is(RO_SHUFFLE_GRASS_OVERWORLD) || grassSetting.Is(RO_SHUFFLE_GRASS_ALL);
}

void EnWood02_RandomizerInit(void* actorRef) {
    EnWood02* treeActor = static_cast<EnWood02*>(actorRef);
    const bool bushesEnabled = Rando::Context::GetInstance()->GetOption(RSK_SHUFFLE_BUSHES).Get() ||
                               GrassSanityIncludesBushes();
    if ((treeActor->actor.params <= WOOD_TREE_KAKARIKO_ADULT &&
        ...
        (treeActor->actor.params > WOOD_TREE_KAKARIKO_ADULT &&
         treeActor->actor.params <= WOOD_BUSH_BLACK_LARGE_SPAWNED && bushesEnabled)) {
        auto treeIdentity =
        ...
        if (treeIdentity.randomizerInf != RAND_INF_MAX && treeIdentity.randomizerCheck != RC_UNKNOWN_CHECK) {
            // A sanity check being complete must not permanently delete the
            // physical bush. Keep its identity attached on reload; the normal
            // EnWood02_RandomizerHoldsItem() check already returns false once
            // randomizerInf is set, so checked bushes simply behave as normal
            // resource bushes and can never pay the AP location twice.
...
void RegisterShuffleTrees() {
    ...
    bool shouldRegisterBush = IS_RANDO &&
...
void Rando::StaticData::RegisterTreeLocations() {
    ...
    locationTable[RC_KAK_TREE]                    = Location::Tree(RC_KAK_TREE,                          RCQUEST_BOTH, RCAREA_KAKARIKO_VILLAGE,      SCENE_KAKARIKO_VILLAGE,            TWO_ACTOR_PARAMS(-860, 522),    "Tree",                              RHT_TREE_KAKARIKO,        RG_DEKU_NUTS_5, SpoilerCollectionCheck::RandomizerInf(RAND_INF_KAK_TREE));
    locationTable[RC_LLR_TREE]                    = Location::Tree(RC_LLR_TREE,                          RCQUEST_BOTH, RCAREA_LON_LON_RANCH,         SCENE_LON_LON_RANCH,               TWO_ACTOR_PARAMS(1309, -2241),  "Tree",                              RHT_TREE_LON_LON_RANCH,   RG_DEKU_NUTS_5, SpoilerCollectionCheck::RandomizerInf(RAND_INF_LLR_TREE));
