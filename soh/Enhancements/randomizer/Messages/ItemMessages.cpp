...
void LoadCustomItemIcon(bool displayAsEnglish) {
    ...
    if (customIcon != nullptr) {
        ...
        char* iconDest = (char*)((uintptr_t)msgCtx->textboxSegment + MESSAGE_STATIC_TEX_SIZE);
        if (rgid == RG_AP_REMOTE_IMPORTANT || rgid == RG_AP_REMOTE_NORMAL) {
            // Cross-world AP icons are custom resources. Resolve them NOW and copy
            // stable raw RGBA32 bytes into textboxSegment instead of storing an
            // __OTR__ token there. This prevents Fast3D from later dereferencing a
            // stale/overwritten resource-name pointer when the pause/menu state
            // changes while the item textbox exists.
            char* pixels = ResourceMgr_LoadTexOrDListByName(customIcon);
            const size_t side = iconSize == ICON_SIZE_24 ? 24u : 32u;
            const size_t byteCount = side * side * 4u;
            if (pixels != nullptr) {
                std::memcpy(iconDest, pixels, byteCount);
            } else {
                // Fail safe: no icon is safer than emitting a bad texture address.
                std::memset(iconDest, 0, byteCount);
            }
        } else {
            // Preserve Ship's resource-token behavior for existing native custom icons.
            std::strcpy(iconDest, customIcon);
        }
        msgCtx->msgBufPos++;
        msgCtx->choiceNum = 1;
    }
