...
std::vector<RandomizerCheck> ReachabilitySearch(const std::vector<RandomizerCheck>& targetLocations,
    ...
    // SOH-EXTREME Check Finder can deliberately search from RR_ROOT so every
    // zone entrance condition is enforced.  When it does, preserve the live
    // player's age/time instead of silently falling back to the seed's original
    // starting age.  This keeps Adult-only checks visible for an Adult player
    // while still requiring the full Root -> ... -> parent-region route.
    if (calculatingAvailableChecks && startingRegion == RR_ROOT && startingAgeTime != RAT_NONE) {
        Region* root = RegionTable(RR_ROOT);
        root->childDay = false;
        root->childNight = false;
        root->adultDay = false;
        root->adultNight = false;

        if (startingAgeTime == RAT_CHILD_DAY) {
            root->childDay = true;
        } else if (startingAgeTime == RAT_CHILD_NIGHT) {
            root->childNight = true;
        } else if (startingAgeTime == RAT_ADULT_DAY) {
            root->adultDay = true;
        } else if (startingAgeTime == RAT_ADULT_NIGHT) {
            root->adultNight = true;
        }
    }
    ...
    const bool previousCalculatingAvailableChecks = logic->CalculatingAvailableChecks;
    ...
    // Available-check calculation is temporary. Leaving this flag set caused
    // later price/item/logic queries to run in Check Finder mode and disagree
    // with normal randomizer generation. Restore the caller's state before
    // returning.
    logic->CalculatingAvailableChecks = previousCalculatingAvailableChecks;
    return gals.accessibleLocations;
}

// SOH-EXTREME 0.8.09: Full-access validation for the native randomizer.
// A seed is not accepted merely because the goal is beatable. When
// All Locations Reachable is selected, every enabled randomized location must
// become reachable by collecting the items that are actually placed in the
// world. This uses the same Region/LocationAccess engine as generation and
// Check Finder, including Souls, abilities, age/time, Silver Rupees, keys,
// prices and room/entrance gates.
static bool ValidateAllRandomizedLocationsReachable() {
    auto ctx = Rando::Context::GetInstance();
    ...
    do {
        gals.InitLoop();
        for (size_t i = 0; i < gals.regionPool.size(); i++) {
            ProcessRegion(RegionTable(gals.regionPool[i]), gals, RG_NONE, false, false);
        }
    } while (gals.logicUpdated);
    ...
    for (RandomizerCheck rc : ctx->allLocations) {
        ...
        if (itemLocation == nullptr || itemLocation->IsAddedToPool()) {
            continue;
        }

        // allLocations is already the option-filtered randomized location pool.
        // Report several concrete failures to make logic bugs diagnosable.
        if (unreachableCount < 25) {
            const auto* loc = Rando::StaticData::GetLocation(rc);
            SPDLOG_ERROR("SOH-EXTREME unreachable randomized location: {}",
                         loc != nullptr ? loc->GetName() : "<unknown>");
        }
        unreachableCount++;
    }
    ...
    if (unreachableCount != 0) {
        SPDLOG_ERROR(
            "SOH-EXTREME full-access validation failed: {} randomized location(s) never became reachable",
            unreachableCount);
        return false;
    }
...
#undef LOCATIONS_PER_SHOP
        ...
        bool fullAccessValid = true;
        if (ctx->GetOption(RSK_ALL_LOCATIONS_REACHABLE)) {
            SPDLOG_INFO("Validating that every randomized location is reachable...");
            fullAccessValid = ValidateAllRandomizedLocationsReachable();
            if (!fullAccessValid) {
                placementFailure = true;
            }
        }

        // Successful placement requires both a beatable goal and, when ALR is
        // enabled, a complete all-location progression sweep.
        if (ctx->playthroughBeatable && fullAccessValid && !placementFailure) {
