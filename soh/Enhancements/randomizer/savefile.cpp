...
void StartingItemGive(GetItemEntry getItemEntry, RandomizerCheck randomizerCheck) {
    if (randomizerCheck != RC_MAX) {
        ...
        // Link's Pocket and any other starting check are Archipelago locations.
        // Reporting the check is enough; the AP server will send the actual item.
        if (Archipelago_ShouldHandleCheck(static_cast<int32_t>(randomizerCheck))) {
            SPDLOG_INFO("[Archipelago] Starting RC {} checked; sending immediately and suppressing local starting reward",
                        static_cast<uint32_t>(randomizerCheck));
            Archipelago_ReportCheck(static_cast<int32_t>(randomizerCheck));
            return;
        }
    }
