...
// SOH-EXTREME 0.7.86: Check Finder/generation must require the same
// language that the runtime actor interaction requires. "Any Speak" is not
...
static RandomizerGet MegaSpeakItemForLocation(const Rando::Location* loc) {
    ...
    if (begins("KF ") || begins("LW ") || begins("SFM ") || name.find("Kokiri") != std::string::npos ||
        name.find("Mido") != std::string::npos || name.find("Saria") != std::string::npos ||
        name.find("Skull Kid") != std::string::npos) return RG_SPEAK_KOKIRI;
    if (begins("GC ") || begins("DMT ") || begins("DMC ") || name.find("Goron") != std::string::npos ||
        name.find("Medigoron") != std::string::npos || name.find("Biggoron") != std::string::npos) return RG_SPEAK_GORON;
    if (begins("ZR ") || begins("ZD ") || begins("ZF ") || name.find("Zora") != std::string::npos ||
        name.find("Jabu") != std::string::npos) return RG_SPEAK_ZORA;
    if (begins("GF ") || begins("GV ") || begins("Wasteland ") || begins("Colossus ") ||
        name.find("Gerudo") != std::string::npos || name.find("Carpet Salesman") != std::string::npos) return RG_SPEAK_GERUDO;
    if (name.find("Deku Scrub") != std::string::npos || name.find("Scrub") != std::string::npos) return RG_SPEAK_DEKU;
    return RG_SPEAK_HYLIAN;
}

static bool MegaHasSpeakForLocation(const Rando::Location* loc) {
    auto ctx = Rando::Context::GetInstance();
    if (!ctx->GetOption(RSK_SHUFFLE_SPEAK)) return true;
    return logic->HasItem(MegaSpeakItemForLocation(loc));
}
...
static bool MegaSoulAllowsLocation(RandomizerCheck rc) {
    ...
    // SOH-EXTREME: Shovel is a physical gate for every grotto, including
    // holes that are open/visible in vanilla and the Deku Theater grotto.
    if (ctx->GetOption(RSK_SHUFFLE_SHOVEL) && loc->GetScene() == SCENE_GROTTOS &&
    ...
    // cows, fish, insects, butterflies and horses). In individual-animal mode
    // each actor must use its own soul; checking only RG_ANIMAL_SOUL here made
    // every individual-animal location look unreachable in Check Finder.
    if (ctx->GetOption(RSK_SHUFFLE_ANIMAL_SOUL) && MegaIsAnimalLocationActor(loc->GetActorID())) {
        RandomizerGet requiredAnimalSoul = RG_ANIMAL_SOUL;
        switch (loc->GetActorID()) {
            case ACTOR_EN_COW:    requiredAnimalSoul = RG_ANIMAL_SOUL_COW; break;
            case ACTOR_EN_NIW:    requiredAnimalSoul = RG_ANIMAL_SOUL_CUCCO; break;
            case ACTOR_EN_DOG:    requiredAnimalSoul = RG_ANIMAL_SOUL_DOG; break;
            case ACTOR_EN_FISH:   requiredAnimalSoul = RG_ANIMAL_SOUL_FISH; break;
            case ACTOR_EN_INSECT: requiredAnimalSoul = RG_ANIMAL_SOUL_BUG; break;
            case ACTOR_EN_BUTTE:  requiredAnimalSoul = RG_ANIMAL_SOUL_BUTTERFLY; break;
            case ACTOR_EN_FR:     requiredAnimalSoul = RG_ANIMAL_SOUL_FROG; break;
            case ACTOR_EN_HORSE:
            case ACTOR_EN_HORSE_NORMAL:
                requiredAnimalSoul = RG_ANIMAL_SOUL_HORSE;
                break;
            default:
                break;
        }
        if (!logic->HasAnimalSoul(requiredAnimalSoul)) {
            return false;
        }
    }
    ...
    switch (loc->GetRCType()) {
        ...
        case RCTYPE_MERCHANT:
            ...
            if (!MegaHasSpeakForLocation(loc)) return false;
        ...
        case RCTYPE_CRATE:
        case RCTYPE_NLCRATE:
            // Crate Soul is only the unlock/existence gate. The player must
            // still have a real physical method to break the crate.
            return logic->CanBreakCrates();
        case RCTYPE_SMALL_CRATE:
            return logic->CanBreakSmallCrates();
        case RCTYPE_GRASS:
        ...
        case RCTYPE_SKULL_TOKEN:
            ...
            return true;
        case RCTYPE_COW:
            return logic->HasAnimalSoul(RG_ANIMAL_SOUL_COW);
        case RCTYPE_FISH:
            return logic->HasAnimalSoul(RG_ANIMAL_SOUL_FISH);
        case RCTYPE_BUTTERFLY_FAIRY:
            return logic->HasAnimalSoul(RG_ANIMAL_SOUL_BUTTERFLY);
        default:
            return true;
    }
}
