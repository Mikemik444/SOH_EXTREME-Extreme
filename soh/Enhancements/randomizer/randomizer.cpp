...
extern "C" u16 Randomizer_Item_Give(PlayState* play, GetItemEntry giEntry) {
    ...
    // Combined Speak mode uses one AP/native item while actor speech still
    // checks six independent language flags.
    if (item == RG_SPEAK_HYLIAN &&
        OTRGlobals::Instance->gRandomizer->GetRandoSettingValue(RSK_SHUFFLE_SPEAK) == 1) {
