...
#pragma region Vanilla
    ...
    areaTable[RR_ICE_CAVERN_FINAL_ROOM] = Region("Ice Cavern Final Room", SCENE_ICE_CAVERN, {}, {
        ...
        LOCATION(RC_SHEIK_IN_ICE_CAVERN,         (AnyAgeTime([]{return logic->CanKillEnemy(RE_WOLFOS);}) && logic->HasItem(RG_OPEN_CHEST)) && (!ctx->GetOption(RSK_SHUFFLE_NPC_SOUL) || logic->HasItem(RG_NPC_SOUL)) && (!ctx->GetOption(RSK_SHUFFLE_SPEAK) || logic->HasItem(RG_SPEAK_HYLIAN))), // rando enables this for child
    }, {
        //Exits
...
#pragma region MQ
    ...
    areaTable[RR_ICE_CAVERN_MQ_STALFOS_ROOM] = Region("Ice Cavern MQ Stalfos Room", SCENE_ICE_CAVERN, {}, {
        ...
        LOCATION(RC_SHEIK_IN_ICE_CAVERN,            (logic->CanKillEnemy(RE_STALFOS) && logic->HasItem(RG_OPEN_CHEST)) && (!ctx->GetOption(RSK_SHUFFLE_NPC_SOUL) || logic->HasItem(RG_NPC_SOUL)) && (!ctx->GetOption(RSK_SHUFFLE_SPEAK) || logic->HasItem(RG_SPEAK_HYLIAN))), // rando enables this for child
    }, {
        //Exits
