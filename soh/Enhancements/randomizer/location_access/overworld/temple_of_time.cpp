...
void RegionTable_Init_TempleOfTime() {
    ...
    areaTable[RR_TOT_BEYOND_DOOR_OF_TIME] = Region("Beyond Door of Time", SCENE_TEMPLE_OF_TIME, {}, {
        ...
        LOCATION(RC_SHEIK_AT_TEMPLE,  (logic->IsAdult && logic->HasItem(RG_FOREST_MEDALLION)) && (!ctx->GetOption(RSK_SHUFFLE_NPC_SOUL) || logic->HasItem(RG_NPC_SOUL)) && (!ctx->GetOption(RSK_SHUFFLE_SPEAK) || logic->HasItem(RG_SPEAK_HYLIAN))),
    }, {
        //Exits
        ENTRANCE(RR_TEMPLE_OF_TIME, true),
    });
