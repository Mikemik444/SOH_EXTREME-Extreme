...
void SaveManager::LoadRandomizer() {
    ...
    // AP owns item placements and hint contents.  These two native arrays are by
    // far the largest part of a randomizer save (thousands of JSON objects).
    // Read the AP marker first so AP saves can skip rebuilding data that will be
    // restored from Archipelago slot data / LocationScouts anyway.
    bool lightweightArchipelagoSave = false;
    SaveManager::Instance->LoadData("archipelagoSave", lightweightArchipelagoSave, false);
...
void SaveManager::SaveRandomizer(SaveContext* saveContext, int sectionID, bool fullSave) {
    ...
    if (lightweightArchipelagoSave) {
        ...
        // RC_MAX placement objects on every save was the main reason Save->Reset
        // and brand-new file creation spent a long time blocked in ThreadPoolWait.
        // Overwrite the keys with real empty arrays so stale data from a previous
        // in-memory saveBlock cannot survive.
        SaveManager::Instance->SaveArray("itemLocations", 0, [&](size_t) {});
    } else {
    ...
    if (lightweightArchipelagoSave) {
        SaveManager::Instance->SaveArray("hintLocations", 0, [&](size_t) {});
    } else {
...
void SaveManager::SaveSection(int fileNum, int sectionID, bool threaded) {
    ...
    if (threaded) {
        smThreadPool->detach_task(
            std::bind(&SaveManager::SaveFileThreaded, this, fileNum, saveContext, sectionID, archipelagoSaveSnapshot));
    } else {
        SaveFileThreaded(fileNum, saveContext, sectionID, archipelagoSaveSnapshot);
    }
}
