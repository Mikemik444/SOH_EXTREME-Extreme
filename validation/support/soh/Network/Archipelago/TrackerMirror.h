...
constexpr const char* kTrackerVersion = "0.11.21";
struct TrackerRow {
    int64_t id = 0;
...
class TrackerMirrorState {
  public:
    ...
    bool Accept(TrackerSnapshot candidate, uint32_t slot, double now, std::string& error) {
        if (candidate.version != kTrackerVersion) { error = "Tracker version mismatch; use 0.11.21 on both sides."; return false; }
