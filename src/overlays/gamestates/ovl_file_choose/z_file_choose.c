...
void FileChoose_LoadGame(GameState* thisx) {
    ...
    // Archipelago never launches Ship's local randomizer generation thread.
    // Joining that thread here is only meaningful for a normal local-randomizer
    // file. A freshly-created AP file can still have transient randomizer state
    // from file creation, and waiting on it here can block the Start transition
    // until the process is restarted.
    const bool isArchipelagoLoad =
        Archipelago_IsCurrentSaveActive() ||
        (this->buttonIndex >= 0 && this->buttonIndex < 3 &&
         Save_GetSaveMetaInfo(this->buttonIndex)->archipelagoSave);

    if (!isArchipelagoLoad) {
        Randomizer_WaitForGeneration();
    } else {
        osSyncPrintf("[Archipelago] FileChoose_LoadGame: skipping local randomizer generation wait\n");
    }
    ...
    if ((this->buttonIndex == FS_BTN_SELECT_FILE_1 && CVarGetInteger(CVAR_DEVELOPER_TOOLS("DebugEnabled"), 0)) ||
        ...
        if (this->buttonIndex == 0xFE) {
            ...
            if (isArchipelagoLoad) {
                osSyncPrintf("[Archipelago] FileChoose_LoadGame: entering Sram_OpenSave\n");
            }
            Sram_OpenSave();
            if (isArchipelagoLoad) {
                osSyncPrintf("[Archipelago] FileChoose_LoadGame: Sram_OpenSave complete\n");
            }
        }
        if (isArchipelagoLoad) {
            osSyncPrintf("[Archipelago] FileChoose_LoadGame: handing off to Play_Init\n");
        }
