...
static s32 sEnemySpawnPlacements[10];
...
void EnPoField_Init(Actor* thisx, PlayState* play) {
    ...
    if (sNumSpawned != 10) {
        ...
        sEnemySpawnPlacements[sNumSpawned] = MegaSoul_GetEnemyDefeatPlacement(&this->actor);
        sNumSpawned++;
    }
    if (sNumSpawned >= 2) {
        ...
        for (i = 0; i < sNumSpawned; i++) {
            if (fabsf(sSpawnPositions[i].x - player->actor.world.pos.x) < 150.0f &&
                ...
                if (this->actor.world.pos.y != BGCHECK_Y_MIN) {
                    ...
                    MegaSoul_BeginEnemyDefeatLife(&this->actor, sEnemySpawnPlacements[i]);
                    EnPoField_SetupAppear(this);
                    break; // One authored trigger owns this apparition.
                } else {
                    return;
                }
            }
