...
void ObjBombiwa_Update(Actor* thisx, PlayState* play) {
    ...
    if (IS_RANDO && Randomizer_GetSettingValue(RSK_SHUFFLE_ROCK_SOUL) &&
        !Flags_GetRandomizerInf(RAND_INF_ROCK_SOUL)) {
        // Keep the boulder physically blocking the world, but make it immune to
        // explosives/hammer until Rock/Boulder Soul is received.
