...
void BgPoEvent_BlockIdle(BgPoEvent* this, PlayState* play) {
    ...
    if (GameInteractor_Should(VB_AMY_SOLVE, sPuzzleState == 0xF)) {
        ...
        if ((this->type == 0) && (this->index == 0)) {
            amy = MegaSoul_SpawnEnemy(play, &this->dyna.actor, 0, ACTOR_EN_PO_SISTERS, this->dyna.actor.world.pos.x + 30.0f,
...
void BgPoEvent_PaintingPresent(BgPoEvent* this, PlayState* play) {
    ...
    if (((this->timer == 0) || ((thisx->xzDistToPlayer < 150.0f) && (thisx->yDistToPlayer < 50.0f)) ||
        ...
        if (!BgPoEvent_NextPainting(this)) {
            MegaSoul_SpawnEnemy(play, thisx, 0, ACTOR_EN_PO_SISTERS, thisx->world.pos.x, thisx->world.pos.y - 40.0f,
