...
void ObjSwitch_Init(Actor* thisx, PlayState* play) {
    ...
    // SOH-EXTREME: the floor switch model and its DynaPoly must be created at
    // exactly the same final transform.  The old FloorSwitches hook moved the
    // two affected switches *after* DynaPoly_SetBgActor(), so the visible switch
    // could be one unit away from the registered collision depending on which
    // door/room-load path was used.  Resolve the complete transform first and
    // only then register the collision.
    Actor_ProcessInitChain(&this->dyna.actor, sInitChain);

    if (type == OBJSWITCH_TYPE_FLOOR || type == OBJSWITCH_TYPE_FLOOR_RUSTY) {
        // Vanilla floor switches sit one unit above home.  These two placements
        // are the known exceptions handled by Shipwright's FixFloorSwitches
        // enhancement; apply that correction before DynaPoly registration.
        if (this->dyna.actor.params == 0x1200 || this->dyna.actor.params == 0x3A00) {
