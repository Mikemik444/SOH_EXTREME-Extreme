...
#define MOVE_HEIGHT_MIDDLE_CHILD_GRAB 180.0f
...
static f32 BgDdanJd_GetMiddleHeight(void) {
    // SOH-EXTREME: first Progressive Strength is Grab / Power Bracelet.
    // In vanilla, child cannot make the normal moving-platform jump to the
    // Dodongo's Cavern lobby floor switch.  When Grab has been shuffled and
    // obtained, raise only the platform's middle stop slightly so the same
    // vanilla route is a normal child jump instead of requiring recoil/ground
    // jump tricks.  Adult and non-randomizer behavior remain unchanged.
    if (IS_RANDO && LINK_IS_CHILD && Randomizer_GetSettingValue(RSK_SHUFFLE_GRAB) &&
        Flags_GetRandomizerInf(RAND_INF_CAN_GRAB)) {
        return MOVE_HEIGHT_MIDDLE_CHILD_GRAB;
    }
    return MOVE_HEIGHT_MIDDLE;
}
...
void BgDdanJd_Idle(BgDdanJd* this, PlayState* play) {
    ...
    if (this->ySpeed == DEFAULT_Y_SPEED && this->dyna.actor.params < 0x40 &&
        ...
        this->idleTimer = 0;
        this->dyna.actor.world.pos.y = this->dyna.actor.home.pos.y + BgDdanJd_GetMiddleHeight();
    ...
    if (this->idleTimer == 0) {
        ...
        if (this->state == STATE_GO_BOTTOM) {
            this->state = STATE_GO_MIDDLE_FROM_BOTTOM;
            this->targetY = this->dyna.actor.home.pos.y + BgDdanJd_GetMiddleHeight();
