...
void Fishing_UpdateFish(Actor* thisx, PlayState* play2) {
    ...
    // Keep initialized fish alive so Fish Soul received in this room can
    // reveal them immediately. Do not kill/re-spawn them or alter their IDs.
    if (!MegaSoul_ArePondFishPresent()) {
        this->actor.flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
        if (this->actor.params == EN_FISH_AQUARIUM) {
            Lights_PointNoGlowSetInfo(&this->lightInfo, 0, 0, 0, 0, 0, 0, 0);
        }
        return;
    }
    if (this->actor.params == EN_FISH_AQUARIUM) {
...
void Fishing_HandleOwnerDialog(Fishing* this, PlayState* play) {
    // No invisible/prohibited NPC can rent the rod, weigh a fish, give prizes
    // or trigger the first-talk check. Keep the separate scene controller alive.
    if (!MegaSoul_CanTalkToFishingOwner()) {
        this->actor.flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
        return;
    }
...
void Fishing_UpdateOwner(Actor* thisx, PlayState* play2) {
    ...
    if (!MegaSoul_CanTalkToFishingOwner() || (sOwnerTheftTimer != 0) ||
        (Message_GetState(&play->msgCtx) != TEXT_STATE_NONE)) {
        this->actor.flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
    } else {
    ...
    if (MegaSoul_IsFishingOwnerPresent() && !sIsOwnersHatHooked &&
        (sLureEquipped != FS_LURE_SINKING) && (sRodCastState > 0) &&
...
void Fishing_DrawOwner(Actor* thisx, PlayState* play) {
    ...
    if (MegaSoul_IsFishingOwnerPresent() && (this->actor.projectedPos.z < 1500.0f) &&
