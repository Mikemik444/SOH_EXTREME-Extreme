...
void EnFloormas_SetupSmWait(EnFloormas* this) {
    ...
    if ((parent->actionFunc == EnFloormas_SmWait) && (child->actionFunc == EnFloormas_SmWait)) {
        // The three objects form one authored enemy, not three checks. The
        // original may be any member after splitting/merging; select it by
        // immutable placement, never by the mutable circular parent pointer.
        Actor* defeated = &this->actor;
        if (MegaSoul_GetEnemyDefeatPlacement(&parent->actor) >= 0) defeated = &parent->actor;
        else if (MegaSoul_GetEnemyDefeatPlacement(&child->actor) >= 0) defeated = &child->actor;
        // Drop at the last fragment's defeat position while retaining the
        // original actor's immutable check identity. All three are removed next.
        defeated->world.pos = this->actor.world.pos;
        GameInteractor_ExecuteOnEnemyDefeat(defeated);
