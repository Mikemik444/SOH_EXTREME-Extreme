...
void EnHintnuts_Init(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.params == 0xA) {
        ...
        if (this->actor.textId == 0x109B) {
            if (Flags_GetClear(play, 0x9) != 0 && !MegaSoul_IsEnemyDefeatLocationPending(thisx)) {
...
void EnHintnuts_Freeze(EnHintnuts* this, PlayState* play) {
    ...
    if (this->animFlagAndTimer == 0) {
        if (sPuzzleCounter == 3) {
            ...
            this->animFlagAndTimer = 1;
            // The first two puzzle scrubs disappear after the third surrenders.
            // Report each one exactly once, while it is still above the floor.
            // A wrong-order reset (-4) is not a defeat and must not award a check.
            GameInteractor_ExecuteOnEnemyDefeat(&this->actor);
