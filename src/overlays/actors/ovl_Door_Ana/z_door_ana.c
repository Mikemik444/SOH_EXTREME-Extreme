...
static s32 DoorAna_ShovelMissing(DoorAna* this) {
    return IS_RANDO && Randomizer_GetSettingValue(RSK_SHUFFLE_SHOVEL) &&
           !Flags_GetRandomizerInf(RAND_INF_HAS_SHOVEL) && Grotto_ActorRequiresShovel(&this->actor);
}
...
void DoorAna_WaitClosed(DoorAna* this, PlayState* play) {
    ...
    if (DoorAna_ShovelMissing(this)) {
        // Shovel-hidden grottos do not exist visually or interactively yet.
        // Leave the actor dormant so obtaining Shovel can unlock it immediately
        // without requiring the room to be reloaded.
...
void DoorAna_WaitOpen(DoorAna* this, PlayState* play) {
    ...
    if (DoorAna_ShovelMissing(this)) {
        // This also handles open Door_Ana holes that are physically hidden by
        // a boulder/rock in vanilla (for example LW Near Shortcuts Grotto).
        Actor_SetScale(&this->actor, 0.0f);
        this->actor.targetMode = 0;
        return;
    }
