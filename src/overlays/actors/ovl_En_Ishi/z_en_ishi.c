...
// Implemented by ShuffleRocks.cpp. Active shuffled rocks/boulders skip their
// vanilla permanent switch so the physical resource can respawn.
extern s32 Rock_RandomizerShouldRespawn(Actor* actor);
...
static s32 EnIshi_HasRockSoul(void) {
    return !IS_RANDO || !Randomizer_GetSettingValue(RSK_SHUFFLE_ROCK_SOUL) ||
           Flags_GetRandomizerInf(RAND_INF_ROCK_SOUL);
}
...
void EnIshi_Wait(EnIshi* this, PlayState* play) {
    ...
    // Rock/Boulder Soul keeps the rock physically present, but makes it inert.
    // Do not offer lifting and do not accept damaging hits until the Soul exists.
    if (!EnIshi_HasRockSoul()) {
