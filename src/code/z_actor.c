...
void Actor_Kill(Actor* actor) {
    Actor_HookWithEnemySpawnSource(GameInteractor_ExecuteOnActorKill, actor);
    actor->draw = NULL;
    actor->update = NULL;
...
void Actor_Init(Actor* actor, PlayState* play) {
    ...
    if (Object_IsLoaded(&play->objectCtx, actor->objBankIndex)) {
        ...
        if (GameInteractor_ShouldActorInit(actor)) {
            Actor_CallWithEnemySpawnSource(actor->init, actor, play);
            actor->init = NULL;

            Actor_HookWithEnemySpawnSource(GameInteractor_ExecuteOnActorInit, actor);
        } else {
            actor->init = NULL;
            Actor_Kill(actor);
        }
...
void Actor_Destroy(Actor* actor, PlayState* play) {
    if (actor->destroy != NULL) {
        if (GameInteractor_ShouldActorDestroy(actor)) {
            Actor_CallWithEnemySpawnSource(actor->destroy, actor, play);
        }
        actor->destroy = NULL;
    } else {
            ...
            if (actor->init != NULL) {
                ...
                if ((DECR(actor->freezeTimer) == 0) &&
                    ...
                    if (GameInteractor_ShouldActorUpdate(actor)) {
                        Actor_CallWithEnemySpawnSource(actor->update, actor, play);
                        Actor_HookWithEnemySpawnSource(GameInteractor_ExecuteOnActorUpdate, actor);
                    }
...
void Actor_Draw(PlayState* play, Actor* actor) {
    ...
    Actor_CallWithEnemySpawnSource(actor->draw, actor, play);
...
Actor* Actor_Spawn(ActorContext* actorCtx, PlayState* play, s16 actorId, f32 posX, f32 posY, f32 posZ, s16 rotX,
    ...
    const s16 sourceActorIndex = sEnemySceneSpawnIndex;
    Actor* spawnSource = sEnemyExplicitSpawnSource != NULL ? sEnemyExplicitSpawnSource : sEnemyCallbackSource;
    const s32 preparedPlacement = sEnemyPreparedPlacement;
    // Consume ALL one-shot context before allocation/init. Failure and nested
    // births must not leak a scene index, encounter ID or explicit parent.
    sEnemySceneSpawnIndex = -1;
    sEnemyPreparedPlacement = -1;
    sEnemyExplicitSpawnSource = NULL;
    // Actual scene entries are authored placements even when loading a room
    // was requested from a callback. Every unindexed enemy birth is not.
    if (sourceActorIndex >= 0) spawnSource = NULL;
    const bool enemyCreated = MegaSoul_IsEnemySpawnSource(spawnSource);
    const s32 sourcePlacement = enemyCreated ? -1 : (preparedPlacement >= 0 ? preparedPlacement :
        MegaSoul_FindEnemyDefeatSpawn(play->sceneNum, play->roomCtx.curRoom.num, sourceActorIndex,
                                      actorId, (u16)params, posX, posY, posZ));
    ...
    if ((objBankIndex < 0) ||
        ((dbEntry->category == ACTORCAT_ENEMY) && Flags_GetClear(play, play->roomCtx.curRoom.num) &&
         !MegaSoul_IsEnemyDefeatPlacementPending(sourcePlacement) &&
         !(actorId == ACTOR_EN_DH && MegaSoul_HasPendingEnemyInRoom(play->sceneNum,
             play->roomCtx.curRoom.num, ACTOR_EN_DHA)))) {
    ...
    SetActorListIndex(actor, sourceActorIndex);
    // #endregion
    ...
    // Immutable placement capture BEFORE Actor_Init, including original params.
    MegaSoul_CaptureEnemyDefeatIdentityFrom(actor, sourcePlacement, spawnSource);
    temp = gSegments[6];
    Actor_Init(actor, play);
    gSegments[6] = temp;

    Actor_HookWithEnemySpawnSource(GameInteractor_ExecuteOnActorSpawn, actor);
...
Actor* Actor_SpawnAsChild(ActorContext* actorCtx, Actor* parent, PlayState* play, s16 actorId, f32 posX, f32 posY,
    ...
    // parent is normally assigned AFTER Actor_Init. Make it available only to
    // immutable spawn capture first; leave authentic actor linking unchanged.
    sEnemyExplicitSpawnSource = parent;
...
// Scripted enemy producers identify the child BEFORE allocation and room-clear filtering.
// The normal parent/child relationship remains exactly as the original producer requested.
Actor* MegaSoul_SpawnEnemyChild(PlayState* play, Actor* source, s16 slot, s16 actorId,
                                f32 x, f32 y, f32 z, s16 rx, s16 ry, s16 rz, s16 params) {
    sEnemyPreparedPlacement = MegaSoul_FindEnemyDefeatChild(source, actorId, (u16)params, slot);
    Actor* result = Actor_SpawnAsChild(&play->actorCtx, source, play, actorId, x, y, z, rx, ry, rz, params);
    sEnemyPreparedPlacement = -1;
    return result;
}
Actor* MegaSoul_SpawnEnemy(PlayState* play, Actor* source, s16 slot, s16 actorId,
                           f32 x, f32 y, f32 z, s16 rx, s16 ry, s16 rz, s16 params) {
    sEnemyPreparedPlacement = MegaSoul_FindEnemyDefeatChild(source, actorId, (u16)params, slot);
    sEnemyExplicitSpawnSource = source;
    Actor* result = Actor_Spawn(&play->actorCtx, play, actorId, x, y, z, rx, ry, rz, params);
    sEnemyPreparedPlacement = -1;
    return result;
}
...
Actor* Actor_SpawnEntry(ActorContext* actorCtx, ActorEntry* actorEntry, PlayState* play) {
    const int previousMapLoading = gMapLoading;
    gMapLoading = 1;
    Actor* ret = NULL;
...
Actor* Actor_Delete(ActorContext* actorCtx, Actor* actor, PlayState* play) {
    ...
    Actor_HookWithEnemySpawnSource(GameInteractor_ExecuteOnActorDestroy, actor);
