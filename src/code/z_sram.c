...
void Sram_InitSave(FileChooseContext* fileChooseCtx) {
    ...
    const bool isArchipelagoNewFile = currentQuest == QUEST_ARCHIPELAGO;

    if (isArchipelagoNewFile) {
    ...
    // Normal save creation historically waits here so file-select metadata is
    // completely flushed before continuing.  For a brand-new AP file this wait
    // was blocking the file-select/game transition on the very large randomizer
    // JSON section.  The save worker owns a copied SaveContext and can finish in
    // the background; AP's server-side slot remains the authoritative source for
    // placements/settings.  The optimized AP randomizer serializer also makes
    // this worker substantially shorter.
    if (!isArchipelagoNewFile) {
