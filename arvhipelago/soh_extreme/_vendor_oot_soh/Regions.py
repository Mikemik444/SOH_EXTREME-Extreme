...
from .location_access.dungeons import \
    ...
    spirit_temple, \
    water_temple
from .SongShuffle import song_vanilla_locations, get_shuffled_songs, individual_notes_enabled
...
def place_locked_items(world: "SohWorld") -> None:
    ...
    if not world.options.skip_child_zelda:
        ...
        # This is an automatic starting reward, not a visit to Impa. Moving the
        # location must also discard its old physical-interaction rule.
        world.set_rule(world.get_location(Locations.SONG_FROM_IMPA), True_())
        # Keep the physical courtyard: EXTREME places window checks there even
        # when Zelda's quest is skipped. It must never fall back to Menu or to
        # the ungated castle front simply because this starting reward moved.

        
    if world.options.shuffle_songs == "off" and not individual_notes_enabled(world):
