...
AP_RequestStatus AP_SetGiftBoxProperties(AP_GiftBoxProperties props) {
    ...
    std::string LocalGiftBoxDef_s = Json::FastWriter().write(Json::objectValue);
    ...
    std::string GlobalGiftBox_s = Json::FastWriter().write(GlobalGiftBox);
...
void handleGiftAPISetReply(const AP_SetReply& reply) {
    if (reply.key == AP_PLAYER_GIFTBOX_KEY) {
        ...
        Json::Reader().parse(*(std::string*)reply.value, local_giftbox);
        ...
        Json::Reader().parse(*(std::string*)reply.value, json_data);
...
AP_RequestStatus sendGiftInternal(const AP_Gift& gift) {
    ...
    std::string gift_s = Json::FastWriter().write(player_box_update);
    ...
    std::string defVal_s = Json::FastWriter().write(defVal);
