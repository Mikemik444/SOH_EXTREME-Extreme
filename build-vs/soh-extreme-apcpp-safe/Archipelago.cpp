...
void AP_Init(const char* filename) {
    ...
    Json::Reader().parse(mwfile,sp_ap_root);
    mwfile.close();
    sp_save_path = std::string(filename) + ".save";
    std::ifstream savefile(sp_save_path);
    Json::Reader().parse(savefile, sp_save_root);
    savefile.close();
...
void AP_Start() {
    ...
    if (multiworld) {
        ...
        std::string req;
        parse_response(Json::FastWriter().write(fake_msg), req);
        fake_msg.clear();
...
void AP_Shutdown() {
    ...
    sp_save_path.clear();
    sp_save_root.clear();
    // Pending requests were cancelled above, after the socket stopped.
...
void AP_SendItem(std::set<int64_t> const& locations) {
    ...
    if (multiworld) {
        ...
        APSend(Json::FastWriter().write(req_t));
    } else {
...
void AP_DeathLinkSend(const std::string &cause) {
    ...
    b.data = Json::FastWriter().write(v);
    b.games = nullptr;
    b.slots = nullptr;
...
void AP_BulkSetServerData(AP_SetServerDataRequest* request) {
    ...
    std::lock_guard<std::recursive_mutex> sohGuard(sSoHApServerDataMutex);
    ...
    switch (request->type) {
        ...
        default:
            for (int i = 0; i < request->operations.size(); i++) {
                req_t["operations"][i]["operation"] = request->operations[i].operation;
                Json::Value data;
                Json::Reader().parse((*(std::string*)request->operations[i].value), data);
                req_t["operations"][i]["value"] = data;
            }
            if (request->default_value != nullptr) {
                Json::Value default_val_json;
                Json::Reader().parse(*((std::string*)request->default_value), default_val_json);
...
void AP_CommitServerData() {
    ...
    while (!queue_server_data.empty()) {
        const auto request = queue_server_data.front();
        // Pop BEFORE resolving an offline request: resolution can re-enter
        // this function through SetReply and must not process the same front.
        queue_server_data.pop();
        req.append(request.first);
        const std::string command = request.first["cmd"].asString();
        if ((command == "Set" || command == "SetNotify") && request.second != nullptr)
            *request.second = AP_RequestStatus::Done;
        if (!multiworld) resolveDataStorageOp(request.first);
    }
    if (multiworld && !req.empty()) APSend(Json::FastWriter().write(req));
}
...
void AP_BulkGetServerData(AP_GetServerDataRequest* request) {
    ...
    const auto pending = map_server_data.find(request->key);
    if (pending != map_server_data.end()) {
        // Do not orphan a second caller's request in Pending forever.
        if (pending->second != request) request->status = AP_RequestStatus::Error;
        return;
    }
...
void AP_SendBounce(AP_Bounce bounce) {
    ...
    Json::Value data;
    Json::Reader().parse(bounce.data, data);
    req_t[0]["data"] = data;
    APSend(Json::FastWriter().write(req_t));
}
...
void AP_Init_Generic() {
    logfunc("AP: SOH-EXTREME request safety patch 1 active");
    ...
    for (unsigned int i = 0; i < root.size(); i++) {
        ...
        if (cmd == "RoomInfo") {
            ...
            if (!auth) {
                ...
                request = Json::FastWriter().write(req_t);
                return true;
            }
            ...
            for (auto slot_itr = root[i]["slot_data"].begin(); slot_itr != root[i]["slot_data"].end(); ++slot_itr) {
                ...
                if (map_slotdata_callback_int.count(key)) {
                    ...
                    map_slotdata_callback_raw[key](Json::FastWriter().write(*slot_itr));
            ...
            for (auto itr : root[i]["keys"].getMemberNames()) {
                ...
                if (target == nullptr || target->value == nullptr) {
                    if (target != nullptr) target->status = AP_RequestStatus::Error;
                ...
                switch (target->type) {
                    ...
                    case AP_DataType::Double:
                        *((double*)target->value) = root[i]["keys"][itr].asDouble();
                        break;
                    case AP_DataType::Raw:
                        *((std::string*)target->value) = Json::FastWriter().write(root[i]["keys"][itr]);
                        break;
                }
            ...
            if (gifting_supported && root[i]["key"].asString().rfind("GiftBox", 0) == 0) {
                ...
                raw_val =  Json::FastWriter().write(root[i]["value"]);
                raw_orig_val = Json::FastWriter().write(root[i]["original_value"]);
            ...
            if (setreplyfunc) {
                ...
                AP_SetReply setreply;
                setreply.key = root[i]["key"].asString();
                switch (SoHApReplyType(setreply.key)) {
                    case AP_DataType::Int:
            ...
            if (!bouncedfunc) {
                ...
                bounce.data = Json::FastWriter().write(root[i]["data"]);
                bouncedfunc(bounce);
            }
...
void WriteFileJSON(Json::Value val, std::string path) {
    ...
    out.open(path);
    out.seekp(0);
    out << Json::FastWriter().write(val).c_str();
    out.flush();
    out.close();
}
...
bool loadDataPkg(const std::string& game, const std::string& hash) {
    ...
    try {
        Json::Value datapkg;
        Json::Reader().parse(cache_file, datapkg);
