...
def set_region_rules(world: "SohWorld") -> None:
    ...
    add_events(Regions.GORON_CITY, world, [
        ...
        (EventLocations.GC_BUG_ROCK, Events.CAN_ACCESS_BUGS, lambda bundle: blast_or_smash(
            bundle) | can_use(Items.SILVER_GAUNTLETS, bundle)),
        (EventLocations.GC_FIRE_AROUND_POT, LocalEvents.GC_CHILD_FIRE_LIT,
         lambda bundle: is_child(bundle) & can_use(Items.DINS_FIRE, bundle)),
        (EventLocations.GC_WOODS_WARP, LocalEvents.GC_WOODS_WARP_OPEN, lambda bundle: can_detonate_upright_bomb_flower(
            bundle) | can_use(Items.MEGATON_HAMMER, bundle) | has_item(LocalEvents.GC_CHILD_FIRE_LIT, bundle)),
        (EventLocations.GC_DARUNIAS_DOOR_AS_CHILD, LocalEvents.GC_DARUNIAS_DOOR_OPENED_AS_CHILD,
         lambda bundle: is_child(bundle) & can_use(Items.ZELDAS_LULLABY, bundle)),
        (EventLocations.GC_STOP_ROLLING_GORON_AS_ADULT, LocalEvents.GC_STOP_ROLLING_GORON_AS_ADULT, lambda bundle: is_adult(bundle) & can_interact_npc(bundle, "Goron") & (has_item(Items.GORONS_BRACELET, bundle) | has_explosives(bundle) | can_use(
            Items.FAIRY_BOW, bundle) | (can_do_trick(Tricks.GC_LINK_GORON_DINS, bundle) & (can_use(Items.DINS_FIRE, bundle) | (can_do_trick(Tricks.BLUE_FIRE_MUD_WALLS, bundle) & can_use(Items.BOTTLE_WITH_BLUE_FIRE, bundle))))))
    ])
    # Locations
    add_locations(Regions.GORON_CITY, world, [
        (Locations.GC_MAZE_LEFT_CHEST, lambda bundle: can_use_any([Items.MEGATON_HAMMER, Items.SILVER_GAUNTLETS], bundle) | (
            can_do_trick(Tricks.GC_LEFTMOST, bundle) & has_explosives(bundle) & can_use(Items.HOVER_BOOTS, bundle))),
        (Locations.GC_MAZE_CENTER_CHEST, lambda bundle: blast_or_smash(
            bundle) | can_use(Items.SILVER_GAUNTLETS, bundle)),
        (Locations.GC_MAZE_RIGHT_CHEST, lambda bundle: blast_or_smash(
            bundle) | can_use(Items.SILVER_GAUNTLETS, bundle)),
        (Locations.GC_POT_FREESTANDING_POH, lambda bundle: is_child(bundle) & has_item(LocalEvents.GC_CHILD_FIRE_LIT, bundle) & (can_use(Items.BOMB_BAG, bundle) | (
            has_item(Items.GORONS_BRACELET, bundle) & can_do_trick(Tricks.GC_POT_STRENGTH, bundle)) | (can_use(Items.BOMBCHUS_5, bundle) & can_do_trick(Tricks.GC_POT, bundle)))),
        (Locations.GC_ROLLING_GORON_AS_CHILD, lambda bundle: is_child(bundle) & (has_explosives(bundle) | (
            has_item(Items.GORONS_BRACELET, bundle) & can_do_trick(Tricks.GC_ROLLING_STRENGTH, bundle)))),
        ...
        (Locations.GC_MAZE_GOSSIP_STONE_BIG_FAIRY, lambda bundle: (blast_or_smash(bundle) | can_use(
            Items.SILVER_GAUNTLETS, bundle)) & can_use(Items.SONG_OF_STORMS, bundle)),
        ...
        (Locations.GC_MAZE_CRATE, lambda bundle: blast_or_smash(bundle) | (
            can_use(Items.SILVER_GAUNTLETS, bundle) & can_break_crates(bundle)))
    ])
    # Connections
    connect_regions(Regions.GORON_CITY, world, [
        ...
        (Regions.GC_MEDIGORON, lambda bundle: can_break_mud_walls(
            bundle) | has_item(Items.GORONS_BRACELET, bundle)),
        (Regions.GC_WOODS_WARP, lambda bundle: has_item(
            LocalEvents.GC_WOODS_WARP_OPEN, bundle)),
        (Regions.GC_SHOP, lambda bundle: (is_adult(bundle) & has_item(LocalEvents.GC_STOP_ROLLING_GORON_AS_ADULT, bundle)) | (is_child(bundle) & (
    ...
    add_events(Regions.GC_WOODS_WARP, world, [
        (EventLocations.GC_WOODS_WARP_FROM_WOODS, LocalEvents.GC_WOODS_WARP_OPEN,
         lambda bundle: blast_or_smash(bundle) | can_use(Items.DINS_FIRE, bundle))
    ])
    # Connections
    ...
    add_events(Regions.GC_DARUNIAS_CHAMBER, world, [
        (EventLocations.GC_DARUNIAS_CHAMBER_TORCH, LocalEvents.GC_CHILD_FIRE_LIT,
         lambda bundle: is_child(bundle) & can_use(Items.STICKS, bundle))
    ])
    # Locations
    ...
    connect_regions(Regions.GC_DARUNIAS_CHAMBER, world, [
        (Regions.GORON_CITY, lambda bundle: True_()),
        (Regions.DMC_LOWER_LOCAL, lambda bundle: is_adult(bundle) & can_grab(bundle))
    ])
    ...
    connect_regions(Regions.GC_GROTTO, world, [
        (Regions.GC_GROTTO_PLATFORM, lambda bundle: True_())
    ])
